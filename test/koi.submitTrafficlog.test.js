...
const submitTrafficLog   = require('../src/SubmitTrafficlog.js');
...
async function testSubmitTrafficLog () {
  ...
  var action = {
      input : {
        "batchTxId":"jdhjdjsf7777e7e",
        "gateWayUrl": "52485hdhhdhd",
        "stakeAmount": 34          
      },
      caller : "FeSD9TV8aB0GK0yby8A40KEX1N-3wrJQTDbRW4uUiEA"
  }
  var resgistering = submitTrafficLog(STATE, action)

  //console.log('result', resgistering)
  console.log('result', resgistering.state.votes)
  console.log('result', resgistering.state.stateUpdate.trafficLogs.dailyTrafficLog.proposedLogs)
  console.log('result', resgistering.state.stateUpdate.trafficLogs.partcipatesRate)
